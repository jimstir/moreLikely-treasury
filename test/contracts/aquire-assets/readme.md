...
- **Live SDK Integration:** The test utilizes the Uniswap SDK locally to fetch pool data and generate real-time Universal Router `swapCallData` for the WETH -> USDC pair, bypassing unreliable testnet APIs. (See `note.md` for details on SDK vs API usage).
...
### 2. should execute a manual owner live swap (USDC -> WETH) using Uniswap SDK
...
- **Funding the Stakeholder:** Uses the live Uniswap SDK to seamlessly swap the stakeholder's Sepolia ETH directly into Sepolia USDC via the Universal Router.
...
- **WETH Address:** `0xfFf9976782d46CC05630D1f6eBAb18b2324d6B14` (Active Sepolia WETH)
...
- **Proposal & Execution:** Owner proposes swapping 10 USDC for WETH. Stakeholders vote. Upon approval, the owner pulls a live Uniswap SDK route and executes `executeSwap` manually directly on the policy contract.
