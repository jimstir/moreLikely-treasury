...
contract GaslessVotingPolicy is Ownable {
    ...
    bytes32 public constant VOTE_TYPEHASH = keccak256("Vote(address voter,uint256 proposalId,uint256 amount,bool support,uint256 nonce)");
    ...
    function batchVoteDirect(
        ...
        for (uint256 i = 0; i < votes.length; i++) {
            ...
            (bytes32 r, bytes32 s, uint8 v) = splitSignature(voteInfo.signature);
            address recoveredVoter = ecrecover(hash, v, r, s);
            require(recoveredVoter == voteInfo.voter, "Invalid voter signature");

            // 2. Pull voter's TreasuryTokens to policy contract
            IERC20(treasuryToken).safeTransferFrom(voteInfo.voter, address(this), voteInfo.amount);
            ...
            ITreasuryVault(treasuryVault).proposalDeposit(voteInfo.amount, voteInfo.voter, proposalId);
        }
        return true;
    }
    ...
    function batchVoteAndDistribute(
        ...
        for (uint256 i = 0; i < votes.length; i++) {
            ...
            uint256 shares = ITreasuryVault(treasuryVault).proposalDeposit(voteInfo.amount, address(this), proposalId);
