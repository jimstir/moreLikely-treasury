...
contract SmartWallet {
    ...
    function forwardProposal(
        ...
        require(block.timestamp >= lastProposalTime + proposalRateLimit, "Rate limited");
        lastProposalTime = block.timestamp;
        
        uint256 gasStart = gasleft();
        
        uint256 proposalId = vault.proposalOpen(amount, receiver, pOwner, rate, request, token);
    ...
    function forwardExecution(
        ...
        require(allowedPolicies[target] || target == address(vault), "Target not allowed");
    ...
    function _refundGas(uint256 gasStart) internal {
        ...
        uint256 gasPrice = tx.gasprice > maxGasPrice ? maxGasPrice : tx.gasprice;
