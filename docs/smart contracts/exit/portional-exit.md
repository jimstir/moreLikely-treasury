...
## 1. Shareholder Claim Validations (`PortionalExit.sol`)
...
```solidity
require(amount <= eligibleShares[msg.sender], "Exceeds eligible balance");
eligibleShares[msg.sender] -= amount;
```
*   **Verification**: The contract checks the caller's balance against the `eligibleShares` mapping snapshot captured when the exit proposal was initiated.
*   **Double-Claim Protection**: The caller's eligible balance is decremented immediately upon claim.
...
The amount of exit assets dispensed is calculated using the governance-approved `swapRatio` (relative to `1e18` precision):
```solidity
...
## 2. Policy Integration & Attribute Queries (`ITreasuryPolicy`)
...
Before the `PortionalExit` contract is deployed, governance must calculate the fair exchange rate of strategy assets to vault shares.
...
*   **Verification**: The total value is divided by the snapshot's outstanding eligible shares to establish the `swapRatio` passed to the `PortionalExit` constructor.
...
sequenceDiagram
    ...
    Gov->>Exit: Deploy with Vault, proposalNum, swapRatio, and Snapshot
